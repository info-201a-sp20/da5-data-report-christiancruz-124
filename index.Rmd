...
``` {r link, echo = FALSE}
l1_markdown <- "[2018 report](https://www.gunviolencearchive.org/reports/mass-shooting)"
```
...
On `r spfc_date`, a shooting occured in `r city_state`. This incident - that
happened at `r spfc_address` - would become one of the most prominent shootings
of the year, totalling up to `r spfc_injr` and `r spfc_lost`. These 
...
``` {r link, echo = FALSE}
l2_markdown <- "[here](https://www.nytimes.com/2018/06/17/nyregion/trenton-mass-shooting.html)"
```
...
report. The size of the cirle represents the number of victims during the
...
2018 hapened on the east coast, with a very dense incident reports in the 
Maryland/New Jersey/Pennsylvania area. The largest concentration of incident reports on the
west coast are seen in the LA area. However, if we take a look at the same data
from a different angle, we see a very different story.
...
ggplot(data = by_state, aes(x = reorder(state, total), y = total, fill = victim_type)) +
  geom_col() +
  labs(title = "Shooting victims per state (2018)", x = "State", y = "Total victims") +
